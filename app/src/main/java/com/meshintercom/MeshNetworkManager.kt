...
class MeshNetworkManager(private val context: Context) {
    ...
    fun start(secretKey: String, nickname: String) {
        ...
        Log.d(TAG, "Starting Mesh with Service ID: $currentServiceId (Key: $secretKey, Name: $myNickname)")
            ...
            if (payload.type == Payload.Type.BYTES) {
                ...
                if (ttl > 0) {
                    ...
                    if (targets.isNotEmpty()) {
                        val relayPayload = Payload.fromBytes(data) // 'data' now has new TTL in buffer backing array? 
