...
OpusCoder::OpusCoder(int sampleRate, int channels) : sampleRate(sampleRate), channels(channels) {
    ...
    if (error != OPUS_OK) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "Failed to create encoder: %s", opus_strerror(error));
        encoder = nullptr;
    } else {
        // Set VBR
    ...
    if (error != OPUS_OK) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "Failed to create decoder: %s", opus_strerror(error));
        decoder = nullptr;
    }
}
...
int OpusCoder::encode(const int16_t* pcmInput, int frameSize, unsigned char* encodedOutput, int maxOutputBytes) {
    if (!encoder) return -1;
    ...
    if (bytesWritten < 0) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "Encode error: %s", opus_strerror(bytesWritten));
        return -1;
    }
    return bytesWritten;
}

int OpusCoder::decode(const unsigned char* encodedInput, int len, int16_t* pcmOutput, int frameSize) {
    if (!decoder) return -1;
    ...
    if (samplesDecoded < 0) {
        __android_log_print(ANDROID_LOG_ERROR, TAG, "Decode error: %s", opus_strerror(samplesDecoded));
        return -1;
    }
